...
class MicrofluidicsConfig(Config):
    ...
    STEPS_PER_EPOCH = 996 # MAXIMUN(integer) : [number of image] / [Batch size]
...
def detect_and_mask(model, image_path=None, video_path=None): #### Added by Junhong.
    ...
    if image_path:
        ...
        for i in range(repeat):
            ...
            file_name = "mask_{0}_{1}".format(now, video_file_list[i])
            ...
            while pos > 0:
                ...
                print("{0:4d}      i_no_detecionint:{1:3d}, i_detection:{2:3d}, \
                ...
                if ret:
                    ...
                    if N > 0:
                        ...
                        if i_detection_max==False and prev_tmp==False:
                            pos = pos - d*6 # if no mask, skip faster
                        else :
                            pos = pos - d
